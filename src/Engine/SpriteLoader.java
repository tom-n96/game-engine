		...
		try {/*
		   // grassTexture = toCompatibleImage(ImageIO.read(new File("grass.jpg")));
			for(int i =0; i < player.length;i++) {
				...
				player[i]=toCompatibleImage(ImageIO.read(new File("anims/player/"+i+".png")));

			}
			for(int i =0; i < playerforward.length;i++) {

				playerforward[i]=toCompatibleImage(ImageIO.read(new File("anims/playerforward/"+i+".png")));

			}
			for(int i =0; i < playerright.length;i++) {

				playerright[i]=toCompatibleImage(ImageIO.read(new File("anims/playerright/"+i+".png")));

			}
			for(int i =0; i < playerright.length;i++) {

				playerleft[i]=flip(toCompatibleImage(ImageIO.read(new File("anims/playerright/"+i+".png"))));
